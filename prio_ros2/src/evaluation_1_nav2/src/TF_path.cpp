...
public:
    explicit Listener(const std::string& topic_name) : Node("listener"), topic_name_(topic_name) {
        ...
        if (topic_name == "topic_TF" || topic_name == "topic_joint") {
...
private:
    ...
    long getTaskLoad() {
        if (topic_name_ == "topic_TF") {
            return 1105861;
        } else if (topic_name_ == "topic_joint") {
            return 681255;
        } else if (topic_name_ == "topic_URDF") {
            return 802600;
        }
        return 0;
    }
...
int main(int argc, char* argv[]) {
    ...
    std::string topic = "topic_TF"; // Default topic
