...
public:
    explicit Listener(const std::string& topic_name) : Node("listener"), topic_name_(topic_name) {
        ...
        if (topic_name == "topic_lidar" || topic_name == "topic_pos") {
...
private:
    ...
    long getTaskLoad() {
        if (topic_name_ == "topic_lidar") {
            return 222356;
        } else if (topic_name_ == "topic_pos") {
            return 1919866;
        } else if (topic_name_ == "topic_local_costmap") {
            return 934291;
        }
        return 0;
    }
