...
void print_usage() {
    ...
    printf("-t topic_name : Specify the topic on which to subscribe. Defaults to 'topic_camera1'.\n");
}
...
public:
    explicit Listener(const std::string& topic_name) : Node("listener"), topic_name_(topic_name) {
        if (topic_name == "topic_camera1" || topic_name == "topic_processing") {
            pub_ = create_publisher<std_msgs::msg::String>(getNextTopicName(), 10);
        }
...
private:
    ...
    long getTaskLoad() {
        // Customize the task load based on the topic
        if (topic_name_ == "topic_camera1") {
            return 824204; // Example load for camera processing
        } else if (topic_name_ == "topic_processing") {
            return 1473640; // Example load for processing
        } else if (topic_name_ == "topic_detection") {
            return 1857231; // Example load for detection
        }
        return 0;
    }

    std::string getNextTopicName() {
        // Customize the next topic based on the current one
        if (topic_name_ == "topic_camera1") {
            return "topic_processing";
        } else if (topic_name_ == "topic_processing") {
...
int main(int argc, char* argv[]) {
    ...
    std::string topic = "topic_camera1"; // Default topic
