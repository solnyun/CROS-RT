  ...
  : Node("listener")
  {
    ...
    auto callback_goal =
      [this](const std_msgs::msg::String::SharedPtr msg) -> void
      {
		...
		ntp_gettime(&t);
		printf("goal start %s %ld.%09ld\n", msg->data.c_str(), t.time.tv_sec, t.time.tv_usec); 

		dummy_task(20330012);	

		ntp_gettime(&t2);
		printf("goal end %s %ld.%09ld\n", msg->data.c_str(), t2.time.tv_sec, t2.time.tv_usec); 
		syscall(456,2,  msg->data.c_str());
		get_node_base_interface()->data_num = msg->data;
      };

	if (topic_name == "topic_goal"){
		sub_ = create_subscription<std_msgs::msg::String>(topic_name, 10, callback_goal);
	}
  }
...
int main(int argc, char * argv[])
{
  ...
  auto topic = std::string("topic_goal");
