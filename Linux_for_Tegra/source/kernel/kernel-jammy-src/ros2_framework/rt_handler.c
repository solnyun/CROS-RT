...
extern int net_rt_handler_action(void);
extern void wake_up_by_priority(int priority);
...
static int run_rt_handler(void* arg)
{
	...
	printk("run_rt_handler\n");
		...
		//preempt_disable();
                ...
                ret = net_rt_handler_action();

                spin_unlock_irqrestore(&priority_lock, flags);

		//if (ret)
			//wake_up_by_priority(current->rt_priority);
...
static void rt_handler_exit(void)
{
        ...
        //if (krtd) kthread_stop(krtd);
        //if (kmond) kthread_stop(kmond);
