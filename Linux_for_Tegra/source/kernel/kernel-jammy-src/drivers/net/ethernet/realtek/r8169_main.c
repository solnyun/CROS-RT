...
static bool priority_in_use[100] = {false};

	//shkim
static struct task_struct *kmond;
static struct task_struct *krtd;
static struct task_struct *knonrtd;
static int kmond_pid = 0;    //kmond thread (rt-handler) pid
static int krtd_pid = 0;    //krtd thread (rt-handler) pid
static int knonrtd_pid = 0;    //krtd thread (rt-handler) pid
static int highest_pri = 1;  //rt-handler thread priority

static int krtd_on = 0;      //To prevent kernel panic
static int krtd_on_set(const char *val, const struct kernel_param *kp)
{
	int ret = param_set_int(val, kp);
	if (ret == 0) {
		pr_info("krtd_on is set to %d\n", krtd_on);	
		if (krtd_on) {
			kmond = pid_task(find_get_pid(kmond_pid), PIDTYPE_PID);
			krtd = pid_task(find_get_pid(krtd_pid), PIDTYPE_PID);
			knonrtd = pid_task(find_get_pid(knonrtd_pid), PIDTYPE_PID);
		}
	}
	return ret;
}
...
module_param_cb(krtd_on, &my_param_ops, &krtd_on, 0644);
//MODULE_PARAM_DESC(krtd_on, "on/off krtd and also init task structs of k**ds");

int top_half = 0;	//To trace

//To switch mode on run-time
//module_param(krtd_on, int, 0644);
module_param(kmond_pid, int, 0644);
module_param(krtd_pid, int, 0644);
module_param(knonrtd_pid, int, 0644);
//To trace
module_param(top_half, int, S_IWUSR|S_IRUSR);

extern void netif_napi_add_port_priority(struct napi_struct *napi, void (*set_port_priority)(int, int));
extern void netif_napi_add_rt_poll(struct napi_struct *napi, int (*rt_poll)(struct napi_struct*));
extern void netif_napi_add_non_rt_poll(struct napi_struct *napi, void (*non_rt_poll)(struct napi_struct*));
extern void netif_napi_add_non_rt_pl_poll(struct napi_struct *napi, void (*non_rt_pl_poll)(struct napi_struct*, int,int));
extern void netif_napi_add_highest_prio(struct napi_struct *napi, int (*highest_prio)(void));
extern void netif_napi_add_highest_prio(struct napi_struct *napi, int (*highest_prio)(void));
extern void netif_napi_add_remove_port_priority(struct napi_struct *napi, void (*remove_port_priority)(int));
...
/*
	...
	*/
	...
	for (count = 0; count < budget; count++, tp->cur_rx++) {
		...
		if (krtd_on) {
			trace_printk("rtl_rx 1\n");
			//printk("[Start] Message Selector\n");
			//tmp_pri = get_port_priority(port);
			if (port == 7778){
				//skb_queue_tail(tp->non_rt, skb);
				trace_printk("rtl_rx 2\n");
				__skb_queue_tail(tp->non_rt, skb);
				trace_printk("rtl_rx 3\n");
				if (knonrtd->__state != TASK_RUNNING){
					wake_up_process(knonrtd);
				}
			}
			/*
			else if ((tmp_pri > 0) && (tmp_pri < 100)){
				priority_queue_insert(tp->pq, skb, tmp_pri);
				if (highest_pri < tmp_pri){
					highest_pri = tmp_pri;
					if (kmond->__state != TASK_RUNNING)
						wake_up_process(kmond);
				}
				else if (highest_pri == tmp_pri) {
					if (kmond->__state != TASK_RUNNING && krtd->__state != TASK_RUNNING) {
						wake_up_process(kmond);
						//printk("opt 1\n");
					}
				}
			//printk("[End] Message Selector\n");
			}
			*/
			else{
				trace_printk("rtl_rx 4\n");
				napi_gro_receive(&tp->napi, skb);
				trace_printk("rtl_rx 5\n");
			}
		}
		else { 
			//printk("[Start] Default\n");
			napi_gro_receive(&tp->napi, skb);	
			//printk("[End] Default\n");
		}
...
static irqreturn_t rtl8169_interrupt(int irq, void *dev_instance)
{
	...
	if (napi_schedule_prep(&tp->napi)) {
		...
		if (top_half == 1) {
			...
			// 현재 entry에 해당하는 RxDesc 구조체 가져오기
			desc = tp->RxDescArray + entry;
	
			// DMA 메모리로부터 실제 데이터를 가져오기 위한 주소
			addr = le64_to_cpu(desc->addr);
	
			// 해당 데이터를 CPU가 접근할 수 있도록 메모리 동기화
  			dma_sync_single_for_cpu(tp_to_dev(tp), addr, sizeof(struct ethhdr), DMA_FROM_DEVICE);
	
			// 패킷의 첫 번째 몇 바이트를 가져와서 확인
			rx_buf = page_address(tp->Rx_databuff[entry]);
	
			// 헤더의 일부 출력 (예: 14바이트의 Ethernet 헤더 확인)
  			if (rx_buf) {
				struct iphdr *iph = (struct iphdr *)((char *)rx_buf + sizeof(struct ethhdr));
				struct udphdr *uh = (struct udphdr *)((char *)iph + sizeof(struct iphdr));
				// 목적지 포트 번호 확인
				unsigned short port = ntohs(uh->dest);
				if (port == 7411 || port == 7413) {
					printk(KERN_INFO "Top-half: Detected packet with port: %u\n", port);
				}
			}
  
  			// 메모리 다시 동기화 (DMA 장치에 접근하도록 설정)
  			dma_sync_single_for_device(tp_to_dev(tp), addr, sizeof(struct ethhdr), DMA_FROM_DEVICE);
	    }
...
void set_port_priority(int priority, int port)
{
	...
	if (priority < 100)
		priority_in_use[priority] = true;
}

void remove_port_priority(int port)
{
	struct port_priority *entry;
	struct hlist_node *tmp;
	int bkt;
	int priority = get_port_priority(port);

	hash_for_each_safe(port_priority_table, bkt, tmp, entry, node) {
		if (entry->port == port) {
			hash_del(&entry->node);
			kfree(entry);
			printk(KERN_INFO "remove_port_priority port: %d from the table\n", port);
			return;
		}
	}
	//printk(KERN_INFO "Port %d not found in the table\n", port);

	if (priority < 100)
		priority_in_use[priority] = false;
}
...
void priority_queue_purge(struct priority_queue *pq, int priority)
{
	skb_queue_purge(&pq->queue[priority]);
}

int priority_queue_len(struct priority_queue *pq, int priority)
{
	return skb_queue_len(&pq->queue[priority]);
}

int get_next_highest_priority(int current_highest_priority) {
	int i;
	for (i = current_highest_priority - 1; i > 0; i--){
		if (priority_in_use[i]){
			return i;
		}
	}
	return 1;
...
int rt_poll(struct napi_struct *napi)
{
	struct rtl8169_private *tp = container_of(napi, struct rtl8169_private, napi);
	struct sk_buff *tmp_skb = NULL;
	int ret = 0;
	...
	while (tmp_skb != NULL){
		//printk("[Start] RT handler\n");
		napi_gro_receive(&tp->napi, tmp_skb);

		spin_lock_irqsave(&priority_lock, flags);
		tmp_skb = priority_queue_remove(tp->pq, highest_pri);
		//printk("[End] RT handler\n");
		spin_unlock_irqrestore(&priority_lock, flags);
	...
	/*	
	for (i = 2; i < 100; i++) {
		if (priority_in_use[i] && (priority_queue_len(tp->pq, i) > 100)) {
			printk(KERN_INFO "Flushing priority %d due to high load\n", i);
			priority_queue_purge(tp->pq, i);
			break;
		}
	}
	*/
	...
	if (priority_queue_empty(tp->pq, highest_pri)){
		if (highest_pri < 2)
			...
			highest_pri = get_next_highest_priority(highest_pri);
	}
	return ret;
}

void non_rt_poll(struct napi_struct *napi)
{
	struct rtl8169_private *tp = container_of(napi, struct rtl8169_private, napi);
	struct sk_buff *tmp_skb = NULL;
	unsigned long flags;
	...
	//background server
	//[Assumption] The number of core is 1.
	preempt_disable();
...
void non_rt_pl_poll(struct napi_struct *napi, int period, int util)
{
	struct rtl8169_private *tp = container_of(napi, struct rtl8169_private, napi);
	struct sk_buff *tmp_skb = NULL;
	unsigned long flags;
	...
	// util is percentage
	int process_ms = (util * period / 100);
	int process_budget = process_ms / 2;
	ktime_t start, end;
	int elapsed_ms;
	...
	if (elapsed_ms < process_ms)
		mdelay(process_ms - elapsed_ms);


	/*
	if (skb_queue_len(tp->non_rt) > 100){
		printk("overflow!\n");
		//skb_queue_purge(tp->non_rt);
		while ((tmp_skb = __skb_dequeue(tp->non_rt)) != NULL)
			kfree_skb(tmp_skb);
...
static int rtl_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	netif_napi_add_remove_port_priority(&tp->napi, remove_port_priority);
	netif_napi_add_rt_poll(&tp->napi, rt_poll);
	netif_napi_add_non_rt_poll(&tp->napi, non_rt_poll);
	netif_napi_add_non_rt_pl_poll(&tp->napi, non_rt_pl_poll);
